/**
 * System prompts for the three memory workers. Model-facing canonical text —
 * deliberately English regardless of UI language. Adapted from
 * pi-observational-memory's observer/reflector/dropper prompts to DSH's
 * session-event addressing (`[Source event seq: N]` labels).
 */
...
- A new chunk of conversation with source event labels and inline message timestamps. Each source block starts with "[Source event seq: <n>]" followed by content formatted as "[User @ YYYY-MM-DD HH:MM]:", "[Assistant @ ...]:", or "[Tool result for <name> @ ...]:".
...
- For every observation, include sourceEventSeqs: the smallest exact set of "[Source event seq: ...]" numbers that directly support the observation.
- Never invent source event seqs. Use only seqs printed in the chunk. If an observation spans multiple turns or tool results, include every supporting source event seq.
- Observations with missing, empty, or invalid sourceEventSeqs will be rejected and not recorded, so do not call record_observations until you can cite valid source seqs.
