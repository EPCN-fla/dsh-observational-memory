...
/**
 * Deterministic, model-facing usage instructions prepended to every rendered
 * memory block. Kept in English: this text is model-facing canonical
 * instructions, not user-facing UI copy.
 */
...
/**
 * Render the memory block made visible to the agent. Returns an empty string
 * when there is nothing to show, so callers can delegate to the native path.
 * The renderer never rewrites memory content.
 */
